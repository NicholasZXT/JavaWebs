...
/**
 * 1. 使用 @SpringBootApplication 注解标识 SpringBoot 启动入口类，其中的 main方法 中执行 SpringApplication.run(MyApplication.class,args)
 *    来启动SpringBoot应用；
 ...
 *   (3) 如果像这里把配置文件都放在了 resources/springboot-hello 目录下，就必须指定命令行参数
 *       --spring.config.location=classpath:/springboot-hello/，这样才能找到配置文件，注意，末尾的反斜杠也必须要有，表示是文件夹而不是文件。
 *   (4) 可以使用 --spring.config.name 来指定配置文件名称，而不是使用默认的 application.xxx 。
 *   (5) application.properties 配置文件里，尽量不要有中文，否则会导致乱码。
 */
@SpringBootApplication
public class MyApplication {

    // 启动的时候，注意加上命令行参数 --spring.config.location=classpath:/springboot-hello/
    // 这里实际使用的是 springboot-hello/application.yml 配置文件
