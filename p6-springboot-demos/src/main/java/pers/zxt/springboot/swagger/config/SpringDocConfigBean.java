...
/**
 * Springdoc 配置 Swagger-UI 的第 1 种方式，使用 Bean 对象返回配置 —— 推荐使用这种方式，这也是SpringDoc主流配置方式。
 * 此种方式又分为两种：
...
public class SpringDocConfigBean {
    ...
    public OpenAPI customOpenAPI() {
        ...
        Info info = new Info()
                .title("Swagger-UI-3.0(OpenAPI) - From Bean")             // Api接口文档标题（必填）
                .description("Some Description from **Bean**.")           // Api接口文档描述
        ...
        openApi.openapi("3.0.1")  // 设置 Open API版本，默认就是 3.0.1
                ...
                .servers(Collections.singletonList(new Server().url("http://localhost:8100/")));
        return openApi;
    }
